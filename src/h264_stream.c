...
h264_stream_t *h264_new(void)
{
    ...
    stream = (h264_stream_t *)calloc(1, sizeof(h264_stream_t));
    if (stream == NULL) {
        ERR("alloc stream failed.");
        return NULL;
    }

    stream->nal = (nal_t *)calloc(1, sizeof(nal_t));
    if (stream->nal == NULL) {
        ERR("alloc nal failed.");
        goto err;
    }
    stream->aud = (aud_t*)calloc(1, sizeof(aud_t));
    if (stream->sh == NULL) {
        ERR("alloc aud failed.");
        goto err;
    }
    stream->sh = (slice_header_t*)calloc(1, sizeof(slice_header_t));
    if (stream->sh == NULL) {
        ERR("alloc sh failed.");
        goto err;
    }
    ...
    // initialize tables
    for (i = 0; i < SPS_TABLE_SIZE; i++) {
        stream->sps_table[i] = (sps_t*)calloc(1, sizeof(sps_t));
        if (stream->sps_table[i] == NULL) {
            ERR("alloc sps_table %d failed.", i);
            goto err;
        }
    }
    for (i = 0; i < PPS_TABLE_SIZE; i++) {
        stream->pps_table[i] = (pps_t*)calloc(1, sizeof(pps_t));
        if (stream->pps_table[i] == NULL) {
            ERR("alloc pps_table %d failed.", i);
            goto err;
        }
    }

    stream->sps = stream->sps_table[0];
    stream->pps = stream->pps_table[0];
    stream->num_seis = 0;
    stream->seis = NULL;
    stream->sei = NULL;  //This is a TEMP pointer at what's in stream->seis...
    return stream;
...
void h264_free(h264_stream_t* stream)
{
    int i;
    ...
    for (i = 0; i < 32; i++) {
        free(stream->sps_table[i]);
    }
    for (i = 0; i < 256; i++) {
        free(stream->pps_table[i]);
    }
    if(stream->seis != NULL) {
        for(i = 0; i < stream->num_seis; i++)
        {
            sei_free(stream->seis[i]);
        }
        free(stream->seis);
    }
